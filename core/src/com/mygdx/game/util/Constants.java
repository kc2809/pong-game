package com.mygdx.game.util;

public class Constants {
    public static final float VIEWPORT_WIDTH = 7.8f;
    public static final float VIEWPORT_HEIGHT = 10.0f;
    public static final String TEXTURE_ATLAS_OBJECT = "canyonbunny.pack";
    public static final String TEXTURE_ATLAS_NUMBER = "number.pack";
    public static final int PPM = 2000;

    public static short BALL_PHYSIC = 0x01;
    public static short WORLD_PHYSIC = 0x02;

    public static int SQUARE_WIDTH = 1;
    public static int SQUARE_HEIGHT = 1;

    public static final float SPEED = 15.0f;

    public static final float SPACE_BETWEEN_SQUARE = 0.1f;
}
